...
export function AuthShell({ children }: { children: ReactNode }) {
  return (
    <div className="relative min-h-[calc(100vh-4rem)] overflow-hidden">
      ...
      <div className="absolute -top-32 -left-24 size-96 rounded-full bg-honey-400/15 blur-3xl animate-blob" aria-hidden />
      ...
      <div className="relative mx-auto flex min-h-[calc(100vh-4rem)] max-w-6xl flex-col items-center justify-center gap-10 px-4 py-12 sm:px-6 lg:flex-row lg:gap-20 lg:px-8">
        ...
        <div className="hidden max-w-md flex-1 lg:block animate-fade-up">
          <BrandMark />
          <h2 className="mt-8 text-4xl font-bold leading-tight tracking-tight text-slate-900 dark:text-white font-heading text-balance">
          ...
          <ul className="mt-8 space-y-4">
            {[
              ...
              <li key={text} className="flex items-center gap-3 text-sm text-slate-600 dark:text-slate-300">
                <span className="inline-flex size-8 items-center justify-center rounded-lg bg-gradient-to-br from-terra-600 to-rust-600 text-white shadow-sm shadow-terra-600/20">
          ...
          <Link
            href="/"
            className="mt-10 inline-flex items-center gap-1.5 text-sm font-medium text-terra-700 transition-colors hover:text-terra-800 dark:text-terra-400 dark:hover:text-terra-300"
          >
            Back to homepage
